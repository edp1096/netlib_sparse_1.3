...
#define HZ 100 // Hz를 100으로 정의 (Windows의 clock tick)
...
double Time() {
    ...
    if (GetProcessTimes(GetCurrentProcess(), &creationTime, &exitTime, &kernelTime, &userTime)) {
        ...
        // 사용자 시간 반환 (단위: 초)
        return (double)(user.QuadPart / 10000.0) / (double)HZ; // 100나노초에서 초로 변환
    } else {
        return -1.0; // 오류 발생 시 -1 반환
    }
}
...
static ComplexVector CheckOutComplexArray(Count)
int Count;
{
    struct Bin Bin, * pBin;
    ComplexVector Temp;
...
static PrintMatrixErrorMessage(Error)
int  Error;
{
    int Row, Col;
...
static EnlargeVectors(NewSize, Clear)
int NewSize, Clear;
{
...
static char* GetProgramName(Arg0)
char* Arg0;
{
    char* pTail;
...
static int InterpretCommandLine(ArgCount, Args)
int ArgCount;
char* Args[];
{
    int I, FileCount = 0;
...
static int Init(pElement, pInitInfo, Row, Col)
RealNumber* pElement;
char* pInitInfo;
int Row, Col;
{
